...
> 稳定、不频繁更新；权威源 `~/.dsh/memory/global/AGENTS.md`（本段为内联副本，改源后同步此处）。
...
主代理负责范围界定、顶层分解、跨切片合约设计与终审。分流标准：**不产出工件的纯问答/诊断/检索讨论可自行处理**；凡要写入代码/文档等工件的实现类任务，无论大小一律经规划层→执行层流水线——小任务的流程开销是设计成本，不得以「任务小」「场景可控」为由跳过（D35）。分发前经用户确认；子代理指令自包含 + 验收标准；多子代理碰同一文件先协调再编辑。
