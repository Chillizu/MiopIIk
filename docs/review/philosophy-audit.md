...
| 已实测（发现） | D31（监督层涌现缺陷漏报 33%，接受为已知限制，见 §4 缺口 4） |
| 已定（设计/原则） | D32（轻量模式三档 preset）、D33（上游裁判输入通道 = 一手产物 + 复算路径） |
| 弃用/降级 | D14（UI 按钮方向，fork=新会话做不出就地回退）、D13 内 `/retry`（自动重试=原生） |
| 未做/可选 | D23（动态 LSP，S9 可选）；D18 token 出口已自建 `mop_run_stats`（墙钟/工具计数仍原生 trajectory） |
